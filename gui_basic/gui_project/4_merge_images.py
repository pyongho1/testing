...
def add_file():
    files = filedialog.askopenfilenames(
        title="이미지 파일을 선택하세요", filetypes=(("PNG FILE", "*.png"), ("ALL FILE", "*.*")), initialdir=r"C:\Users\pjule\Desktop\python\rpa_basic")  # 최초에 사용자가 지정한 경로를 보여줌
...
def merge_image():
    ...
    dest_path = os.path.join(txt_dest_path.get(), "yong_photo.jpg")
...
def start():
    ...
    if len(txt_dest_path.get()) == 0:
        msgbox.showwarning("경고", "저장 경로를 선책하세요")
        return
...
txt_dest_path.pack(side="left", fill="x", expand=True,
                   padx=5, pady=5, ipady=4)  # 아이패드 높이 변경
